...
public interface AuthService<R> {
	 ...
	 * @return the auth user represent the current user, when the user is illegal, a
	 *         null value will return.
	 */
	AuthUser getAuthUser(R request);

	boolean authTarget(R request, String app, PrivilegeType privilegeType) throws Exception;
	...
	interface AuthUser {
		...
		/**
		 * Query whether current user has the specific privilege to the target, the
		 * target may be an app name or an ip address, or other destination.
		 * <p>
		 * This method will use return value to represent whether user has the specific
		 * privileges to the target, but to throw a RuntimeException to represent no
		 * auth is also a good way.
		 * </p>
		 *
		 * @param target        the target to check
		 * @param privilegeType the privilege type to check
		 * @return if current user has the specific privileges to the target, return
